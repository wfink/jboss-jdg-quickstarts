...
/**
 * Creates a DefaultCacheManager which is configured with a configuration file. <b>Infinispan's
 * libraries need to be bundled with the application.</b>
 * 
 * @author Wolf-Dieter Fink
 */
@ApplicationScoped
public class App2CacheManagerProvider {
   private static final String CONFIG = "appcache-config.xml";
   ...
   public DefaultCacheManager getCacheManager() {
      if (manager == null) {
         ...
         GlobalConfiguration glob = new GlobalConfigurationBuilder().clusteredDefault() // Builds a default clustered
               ...
               .clusterName("ClusterApp1").globalJmxStatistics().allowDuplicateDomains(true).enable() // This method enables the jmx statistics of
         ...
         Configuration cache = new ConfigurationBuilder().clustering().cacheMode(CacheMode.REPL_ASYNC).transaction()
